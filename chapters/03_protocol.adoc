...
.The structure of the info byte
[bytefield]
----
(def boxes-per-row 8)
(def box-width 80)
...
This is a 2 byte LE unsigned number, which address the receiver that will
process these configuration changes. The value is otherwise 0.
...
making any sense out of the emerging patterns, we switched to reading it as an
enum with four values. These describe mostly how the data field will be
interpreted, and are a big part of the statefulness of the protocol. The values
are described in <<cmd-conf-type-enum>>.

[[cmd-conf-type-enum]]
.The enum values of the config command type field
...
| `8` | permanent data
| `9` | permanent state configurations
|===

We distinguish between permanent and temporary data, as the configuration can
also be simply written to RAM instead of SPI, allowing the user to experiment
with (possible faulty) panel/ receiver configurations before committing to them.

===== is_data?

This field is a simple bit flag, declaring that the data field is actual data,
and not meta-data how e.g. following packages should be read, …

This value should always be true if and only if `type=1` or `type=9`.


===== Config Pkg ID

This field is independent of the carrier package ID.

A single (sub-)package is repeated multiple times (independent of the carrier
package) before the next one is being sent. This is done presumably to reduce
the issues stemming from an unreliable carrier. To filter these repeated
packages, the protocol uses the 3 bit config package ID. For each new package it
is increased by one, looping after 7.

NOTE: This value is stable between configuration sequences.

===== Address

This is the 3 byte long big-endian address into virtual memory, that the data
field should be written to (where type is data). In cases where type assumes a
configuration enum value, this could also be static values, or a restricted
address.

===== Data

This 16 byte field contains the actual data that contains 16 bytes of the actual
configuration, further describes the command to be executed or is just gibberish.
